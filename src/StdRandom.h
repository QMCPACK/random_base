 ...
 * This is intended to match the behavior of random::uniform_real_distribution
 * in the boost libraries Its behavor is different from
 * std::uniform_real_distribution which uses std::generate_canonical to generate
 * enough entropy according to the precision of T.
 */
template <typename T> class uniform_real_distribution_as_boost {
public:
  using result_type = T;
  ...
  template <typename RNG> result_type operator()(RNG &eng) {
    return static_cast<result_type>(eng() - eng.min()) /
               (static_cast<result_type>(eng.max() - eng.min()) + 1) *
               (max_ - min_) +
           min_;
  }
...
template <typename T> class StdRandom : public RandomBase<T> {
public:
  ...
  std::unique_ptr<RandomBase<T>> clone() const override {
